...
from vits_models import SynthesizerTrn as TeacherSynthesizerTrn
...
def run(rank, n_gpus, hps_dec, hps_enc, hps_teacher):
    ...
    if rank == 0:
        eval_dataset = TextAudioSpeakerLoader(
            hps_dec.data.validation_files, hps_dec.data
        )
    ...
    net_teacher, _, _, _ = utils.load_checkpoint(
        utils.latest_checkpoint_path(hps_dec.data.teacher_model_dir, "G_*.pth"),
        net_teacher,
        None,
    )
    ...
    net_dec = Decoder(n_speakers=hps_dec.data.n_speakers, **hps_dec.model).cuda(rank)
    ...
    try:
        _, _, _, epoch_str = utils.load_checkpoint(
            utils.latest_checkpoint_path(hps_dec.model_dir, "DECODER_*.pth"),
            net_dec,
            optim_dec,
        )
        _, _, _, epoch_str = utils.load_checkpoint(
            utils.latest_checkpoint_path(hps_dec.model_dir, "DECODER_D_*.pth"),
            net_d,
            optim_d,
        )
    ...
    scheduler_g = torch.optim.lr_scheduler.ExponentialLR(
        optim_dec, gamma=hps_dec.train.lr_decay, last_epoch=epoch_str - 2
    )
    scheduler_d = torch.optim.lr_scheduler.ExponentialLR(
        optim_d, gamma=hps_dec.train.lr_decay, last_epoch=epoch_str - 2
    )
    ...
    for batch_idx, (
        ...
        with torch.no_grad():
            ...
            z_slice, ids_slice = commons.rand_slice_segments(
                z, spec_lengths, net_teacher.module.segment_size
            )
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            y_mel = commons.slice_segments(
                mel, ids_slice, net_teacher.module.segment_size
            )
            ...
            with autocast(enabled=False):
                loss_dec_disc, losses_dec_disc = loss_fn.disc_loss_fn(
                    y_d_hat_r, y_d_hat_g
                )
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                logger.info(
                    [x.item() for x in losses] + [grad_norm_dec, global_step, lr]
                )

                scalar_dict = {
                    "loss/dec/total": loss_dec_all,
                    "loss/disc/total": loss_disc_all,
                    "learning_rate": lr,
                    "grad_norm_dec": grad_norm_dec,
            ...
            if global_step % hps.train.eval_interval == 0:
                ...
                utils.save_checkpoint(
                    ...
                    epoch,
                    os.path.join(hps.model_dir, "DECODER_{}.pth".format(global_step)),
                )
                utils.save_checkpoint(
                    net_d,
                    optim_d,
                    hps.train.learning_rate,
                    epoch,
                    os.path.join(hps.model_dir, "DECODER_D_{}.pth".format(global_step)),
                )
        global_step += 1
