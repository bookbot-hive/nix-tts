...
def run(rank, n_gpus, hps, hps_teacher):
    ...
    net_teacher, _, _, _ = utils.load_checkpoint(
        utils.latest_checkpoint_path(hps.data.teacher_model_dir, "G_*.pth"),
        net_teacher,
        None,
    )
    ...
    try:
        _, _, _, epoch_str = utils.load_checkpoint(
            utils.latest_checkpoint_path(hps.model_dir, "ENCODER_*.pth"),
            net_enc,
            optim_enc,
        )
    ...
    scheduler_enc = torch.optim.lr_scheduler.ExponentialLR(
        optim_enc, gamma=hps.train.lr_decay, last_epoch=epoch_str - 2
    )
...
def train(
    ...
    net_teacher.eval()
    net_enc.train()
    for batch_idx, (x, x_lengths, spec, spec_lengths, _, _, speakers) in enumerate(
        train_loader
    ):
        ...
        with autocast(enabled=hps.train.fp16_run):
            (
                ...
                (attn_h, attn_s, attn_logprob),
                (_, y_mask, _),
                _,
            ) = net_enc(x, x_lengths, spec, spec_lengths, speakers)
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                logger.info(
                    [l.item() for l in losses] + [grad_norm_enc, global_step, lr]
                )

                scalar_dict = {
                    "loss/enc/total": loss_enc_all,
                    "learning_rate": lr,
            ...
            if global_step % hps.train.eval_interval == 0:
                utils.save_checkpoint(
                    ...
                    hps.train.learning_rate,
                    epoch,
                    os.path.join(hps.model_dir, "ENCODER_{}.pth".format(global_step)),
                )
        global_step += 1
